...
\subsubsection{Grammatical Gender and Sex}

\nyi{Address this. Relevant examples: tati: name of Lise's dog with one eye}

tati \ipa{@n@nd@k} 'tati he is here'
...
As we saw in the chapter on morphology, sometimes negation is accomplished with a unit that is below the word. Negation is also accomplished by words. \emph{wak} is especially important. Consider the sentences below

Idea: mundai, mare only in the context of verbs, wak when an existential
...
\ex 
 \gll ei yasu \ipa{@n@swis} biwotuk\sw iw moh\sw iy\ipa{@}iw \\
 i have five three sisters \\
 \trans `I have eight sisters.'

 \ex 
 \gll ei moh\sw iy\ipa{@}iw wak \\
 i sisters \textsc{neg} \\
 \trans `I do not have sisters.'

\end{exe}

In negation of what would have been a copulative predication if Arapesh had a copula (`A is B'), \emph{wak} sometimes simply comes at the end, as in the first pair of forms. In the second form it's worth noting that the pronoun \emph{\ipa{@n@n}} is not present, although it's not clear whether this must be so and that \emph{michael \ipa{@n@n} \ipa{@rmatok\sw wak}} would be wrong.

But in the second pair of forms a form \emph{mundai} appears which has not been seen in any other context. It's unclear why the fourth sentence is not instead \emph{\ipa{@kud@ cok\sw uk\sw i} buk wak}.

Note finally the last set of forms, where when the statement is negated the verb \emph{disappears completely}, leaving the reader to infer the entire verb from context. In this context, where family members are being discussed, perhaps the information loss is minimal, but it could easily be so in different contexts where the verb is not so predictable from its surroundings.
 ...
 \ex 
 \gll lara ok\sw ok\sw \ipa{@rmatok} \\
...
\emph{\textbf{Note: the rest of chapter 4 and 5 have not been changed in this draft. These will be fleshed out in the final draft.}}

 \subsection{Adverbs}

Gamum \ipa{n1k1riyen} (not the reverse!)
